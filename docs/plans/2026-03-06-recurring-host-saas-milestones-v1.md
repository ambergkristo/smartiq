...
5. `M6` telemetry now captures bootstrap, auth completion, session launch/duplicate/resume/complete, and billing upgrade lifecycle events into tenant usage summary, with an internal pilot summary, support-case loop, canonical pilot-summary report generator, and pilot-gate automation available in admin ops/tooling.
